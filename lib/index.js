...
async function publishCommentOnPullRequest(token, commit) {
  var _a;
  ...
  const headSha = commit || pullRequest.head.sha || github2.context.sha;
  core4.info(`Got PR number ${prNumber} with SHA: ${headSha}`);
  const octokit = github2.getOctokit(token);
  ...
  delay(500);
  const comments = commentsSearch.data.filter(({ user, body: body2 }) => (user == null ? void 0 : user.login) === "github-actions" && (body2 == null ? void 0 : body2.startsWith("Test Result Summary")));
  const comment_id = (_a = comments.at(-1)) == null ? void 0 : _a.id;
  const body = `Test Result Summary<br /><h3>Whoopy</h3>`;
  if (comment_id) {
    core4.info(`Updating existing comment: ${comment_id} on PR: #${prNumber}`);
    await octokit.rest.issues.updateComment({ ...github2.context.repo, comment_id, body });
  } else {
    core4.info(`Publishing new comment on PR: #${prNumber}`);
