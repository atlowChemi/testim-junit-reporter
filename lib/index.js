...
function parseProjectTokenDictionary(projectTokenDictionaryStrs) {
  core2.warning(projectTokenDictionaryStrs.toString());
...
async function getTestReports(inputs) {
  ...
  core3.setOutput("total", accumulateResult.totalCount);
  core3.setOutput("passed", accumulateResult.passed);
  core3.setOutput("skipped", accumulateResult.skipped);
  core3.setOutput("failed", accumulateResult.failed);
  core3.setOutput("failedEvaluating", accumulateResult.failedEvaluating);
...
async function attachSummary(accumulateResult, testResults) {
  const table = [
    [
...
async function run() {
  try {
    ...
    if (inputs.failOnFailure && conclusion === "failure") {
      core5.setFailed(`\u274C Tests reported ${accumulateResult.failed} failures`);
    }
    core5.endGroup();
  } catch (error3) {
