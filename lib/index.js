...
async function run() {
  try {
    ...
    try {
      ...
      core5.error(`\u274C Failed creating a check using the provided token. (${error3})`);
      core5.warning(
    ...
    if (supportsJobSummary) {
      try {
        ...
        core5.error(`\u274C Failed to set the summary using the provided token. (${error3})`);
      }
    } else {
      core5.warning(`\u26A0\uFE0F Your environment seems to not support job summaries.`);
    }
    if (inputs.failOnFailure && conclusion === "failure") {
      core5.setFailed(`\u274C Tests reported ${accumulatedResult.failed} failures`);
    }
    core5.endGroup();
  } catch (error3) {
