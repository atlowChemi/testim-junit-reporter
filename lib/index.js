...
Support boolean input list: \`true | True | TRUE | false | False | FALSE\``);
    ...
    exports.startGroup = startGroup4;
    function endGroup4() {
      command_1.issue("endgroup");
    }
    exports.endGroup = endGroup4;
...
var require_utils2 = __commonJS({
  "node_modules/@actions/github/lib/internal/utils.js"(exports) {
    ...
    var httpClient2 = __importStar(require_lib());
...
function findMatchingOption(message, ...strings) {
  for (const str2 of strings) {
    ...
    if (index2 > -1) {
      return { index: index2, length: str2.length };
    }
  }
  const str = strings.at(-1);
  const index = str ? message.indexOf(str) : -1;
  return { index, length: (str == null ? void 0 : str.length) || 0 };
}
function parseTestimFailureMessage(textContent) {
  const https = "https://";
...
function getTestimSuiteDetails(testCases) {
  const testimResult = testCases.find(({ ["system-out"]: syso }) => syso == null ? void 0 : syso.includes("https://app.testim.io/#/project"));
  const urlParts = testimResult == null ? void 0 : testimResult["system-out"].match(/\/project\/(?<projectId>.*)\/branch\/(?<branch>.*)\/test/);
  if (!(urlParts == null ? void 0 : urlParts.groups)) {
    return {};
  }
  const { projectId, branch } = urlParts.groups;
  return { projectId, branch };
}
function parseProjectTokenDictionary(projectTokenDictionaryStrs) {
  const entries = projectTokenDictionaryStrs.map((str) => str.split(":").map((val2) => val2.trim())).filter(([key, value, ...rest]) => !rest.length || key && value);
  return Object.fromEntries(entries);
}
var httpClient = new http.HttpClient();
var projectBranchTestStatusMap = /* @__PURE__ */ new Map();
async function getTestStatusesFromPublicAPI(testCases, projectTokenDictionaryStrs) {
  ...
  const loadTestList = async () => {
    ...
    const { tests, error: error3 } = JSON.parse(await res.readBody());
    if (error3) {
...
var InvalidGithubTokenError = class extends Error {
};
function parseInputs() {
  core2.startGroup(`\u{1F4D6} Parsing input values`);
  const token = core2.getInput("github_token") || process.env.GITHUB_TOKEN;
  if (!token) {
    throw new InvalidGithubTokenError("\u274C A token is required to execute this action");
  }
  const values = {
    token,
    commit: core2.getInput("commit"),
    jobName: core2.getInput("job_name"),
    summary: core2.getMultilineInput("summary"),
    checkName: core2.getMultilineInput("check_name"),
    reportPaths: core2.getMultilineInput("report_paths"),
    updateCheck: core2.getInput("update_check") === "true",
    requireTests: core2.getInput("require_tests") === "true",
    failOnFailure: core2.getInput("fail_on_failure") === "true",
    projectTokenDictionary: core2.getMultilineInput("project_api_key_map")
  };
  core2.debug(JSON.stringify(values, void 0, 4));
  core2.endGroup();
  return values;
}
...
async function parseFile(file, projectTokenDictionaryStrs) {
  core3.debug(`Parsing file ${file}`);
  const data = fs.readFileSync(file, "utf8");
  const parser = new import_fast_xml_parser.XMLParser({ allowBooleanAttributes: true, ignoreAttributes: false, attributeNamePrefix: "" });
  const report = parser.parse(data);
  return parseSuite(report, file, projectTokenDictionaryStrs);
}
async function parseSuite(report, fileName, projectTokenDictionaryStrs) {
  var _a, _b, _c;
  const testsuite = report.testsuite || ((_a = report.testsuites) == null ? void 0 : _a.testsuite);
  const result = { fileName, name: testsuite == null ? void 0 : testsuite.name, totalCount: 0, skipped: 0, failedEvaluating: parseInt((testsuite == null ? void 0 : testsuite["failure-evaluating"]) || "0") || 0, annotations: [] };
  ...
  for (const { failure, skipped, name, ["system-out"]: systemOut, classname } of testsuite.testcase) {
    ...
    const isTestimTest = systemOut == null ? void 0 : systemOut.startsWith("https://app.testim.io/#/project");
    const { testId } = ((_b = systemOut == null ? void 0 : systemOut.match(/\/test\/(?<testId>.*)\?/)) == null ? void 0 : _b.groups) || {};
    const testStatus = testId && ((_c = testListInfo == null ? void 0 : testListInfo.find(({ _id }) => _id === testId)) == null ? void 0 : _c.testStatus) || "draft";
    ...
    result.annotations.push({
      ...
      message: isTestimTest ? failure ? parseTestimFailureMessage(failure.message) : systemOut : (failure == null ? void 0 : failure.toString()) || "",
      raw_details: `${escapeEmoji(classname)} - ${name}:
(${failure == null ? void 0 : failure.message})`,
...
async function parseTestReports(checkName, summary2, reportPathsGlob, projectTokenDictionaryStrs) {
  core3.debug(`Process test report for: ${reportPathsGlob} (${checkName})`);
  const testResults = [];
  const globber = await glob.create(reportPathsGlob);
  for await (const file of globber.globGenerator()) {
    core3.debug(`Parsing report file: ${file}`);
...
var NoTestsFoundError = class extends Error {
};
async function getTestReports(inputs) {
  core3.startGroup(`\u{1F4E6} Process test results`);
  const reportsCount = inputs.reportPaths.length;
  core3.info(`Retrieved ${reportsCount} report globs/files to process.`);
  const allResults = await Promise.all(
    inputs.reportPaths.map(
      async (_, i) => parseTestReports(
  ...
  core3.setOutput("total", accumulatedResult.totalCount);
  core3.setOutput("passed", accumulatedResult.passed);
  core3.setOutput("skipped", accumulatedResult.skipped);
  core3.setOutput("failed", accumulatedResult.failed);
  core3.setOutput("failedEvaluating", accumulatedResult.failedEvaluating);
  ...
  const conclusion = actualFailed <= 0 ? "success" : "failure";
...
async function annotateTestResult(testResult, token, headSha, updateCheck, jobName) {
  ...
  core4.info(`\u2139\uFE0F - ${testResult.checkName} - ${title}`);
  const conclusion = testResult.failed <= 0 ? "success" : "failure";
  for (const annotation of annotations) {
    core4.info(`   \u{1F9EA} - ${annotation.message.split("\n", 1)[0]}`);
  }
  ...
  if (updateCheck) {
    ...
    core4.debug(JSON.stringify(checks, null, 2));
    const check_run_id = checks.data.check_runs[0].id;
    core4.info(`\u2139\uFE0F - ${testResult.checkName} - Updating checks ${annotations.length}`);
    for (let i = 0; i < annotations.length; i = i + 50) {
      ...
      core4.debug(JSON.stringify(updateCheckRequest, null, 2));
    ...
    core4.debug(JSON.stringify(createCheckRequest, null, 2));
    core4.info(`\u2139\uFE0F - ${testResult.checkName} - Creating check for`);
    await octokit.rest.checks.create(createCheckRequest);
  }
}
var statusToColorDictionary = {
  draft: "\u{1F535}",
  active: "\u{1F7E2}",
  quarantine: "\u{1F534}",
  evaluating: "\u{1F7E1}"
};
async function attachSummary(accumulatedResult, testResults) {
  const table = [
    [
  ...
  for (const testResult of testResults) {
    ...
    for (const annotation of annotations) {
      const color = statusToColorDictionary[annotation.testStatus];
      detailsTable.push([
        `${testResult.checkName}`,
  ...
  await core4.summary.addHeading("Overall").addTable(table).addSeparator().addHeading("Details").addTable(detailsTable).write();
}
async function publishAnnotations(inputs, { accumulatedResult, testResults, conclusion, headSha }) {
  core4.startGroup(`\u{1F680} Publish results`);
  try {
    for (const testResult of testResults) {
      await annotateTestResult(testResult, inputs.token, headSha, inputs.updateCheck, inputs.jobName);
    }
  } catch (error3) {
    core4.error(`\u274C Failed creating a check using the provided token. (${error3})`);
    core4.warning(
  ...
  if (supportsJobSummary) {
    try {
      ...
      core4.error(`\u274C Failed to set the summary using the provided token. (${error3})`);
    }
  } else {
    core4.warning(`\u26A0\uFE0F Your environment seems to not support job summaries.`);
  }
  if (inputs.failOnFailure && conclusion === "failure") {
    core4.setFailed(`\u274C Tests reported ${accumulatedResult.failed} failures`);
  }
  core4.endGroup();
}
async function publishComment(token) {
  var _a;
  const octokit = github2.getOctokit(token);
  const prSearch = await octokit.rest.search.issuesAndPullRequests({ q: `type:pr repo:${github2.context.repo.owner}/${github2.context.repo.repo} ${github2.context.sha}` });
  core4.info(`found ${prSearch.data.total_count} matching items at ${prSearch.url} with status ${prSearch.status}`);
  const pulls = prSearch.data.items.filter(({ repository, state }) => {
    const isCurrentRepo = (repository == null ? void 0 : repository.full_name) === `${github2.context.repo.owner}/${github2.context.repo.repo}`;
    const isOpen = state === "open";
    return isCurrentRepo && isOpen;
  });
  console.log(prSearch.data.items[0]);
  core4.info(`found ${pulls.length} PRs from the correct repo which are open.`);
  for (const pull of pulls) {
    const commentsSearch = await octokit.rest.pulls.listReviewComments({
      ...github2.context.repo,
      pull_number: pull.number
    });
    core4.info(`found ${commentsSearch.data.length} comments at ${commentsSearch.url} with status ${commentsSearch.status}`);
    delay(500);
    const comments = commentsSearch.data.filter(({ user, body: body2 }) => user.login === "github-actions" && body2.startsWith("Test Result Summary"));
    const comment_id = (_a = comments.at(-1)) == null ? void 0 : _a.id;
    const body = `Test Result Summary<br /><h3>Whoopy</h3>`;
    if (comment_id) {
      core4.info(`Updating existing comment: ${comment_id} on PR: #${pull.number}`);
      await octokit.rest.pulls.updateReviewComment({ ...github2.context.repo, comment_id, body });
    } else {
      core4.info(`Publishing new comment on PR: #${pull.number}`);
      await octokit.rest.pulls.createReviewComment({ ...github2.context.repo, pull_number: pull.number, body });
    }
  }
}
