...
async function publishCommentOnPullRequest(token, { accumulatedResult, testResults, conclusion, headSha }) {
  var _a;
  console.log("DEBUG", github2.context.job, github2.context.action, github2.context.runId, github2.context.runNumber);
  const pullRequest = github2.context.payload.pull_request || { number: 9 };
  if (!pullRequest) {
    return;
  }
  ...
  const summary2 = `Parsed <b><code>${testResults.length}</code></b> JUnit files, and has ended with status <b><code>${conclusion}</code></b>`;
