...
async function parseTestReports(checkName, summary2, reportPathsGlob) {
  ...
  const res = { annotations: [], checkName, summary: summary2, totalCount: 0, skipped: 0, passed: 0, failed: 0, failedEvaluating: 0 };
...
async function attachSummary(testResults) {
  const table = [
    [
      { data: "", header: true },
  ...
  await core4.summary.addTable(table).write();
  await core4.summary.addTable(detailsTable).write();
}
...
async function run() {
  try {
    ...
    try {
      ...
      core5.error(`\u274C Failed to create checks using the provided token. (${error3})`);
      core5.warning(`\u26A0\uFE0F This usually indicates insufficient permissions. More details: https://github.com/mikepenz/action-junit-report/issues/23`);
    }
