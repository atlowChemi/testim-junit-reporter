...
function parseInputs() {
  core.startGroup(`\u{1F4D6} Parsing input values`);
  ...
  core.debug(JSON.stringify(values, void 0, 4));
  core.endGroup();
  return values;
}
...
function retrieve(name, items, index, total) {
  if (total > 1) {
    if (items.length !== 0 && items.length !== total) {
      core2.warning(`${name} has a different number of items than the 'reportPaths' input. This is usually a bug.`);
    }
    if (items.length === 0) {
      return "";
      ...
      core2.error(`${name} has no valid config for position ${index}.`);
      return "";
    }
  } else if (items.length === 1) {
    return items[0].replace("\n", "");
...
function buildTableBasedOnPreviousTable(commentBody, accumulatedResult) {
  const currentJobRow = [
    github2.context.job,
    `${accumulatedResult.totalCount} run`,
    `${accumulatedResult.passed} passed`,
  ...
  const previousTable = commentBody.split("<table>")[1].replace(/<\/(table|tr|td|th)>/g, "").split("<tr>").filter(Boolean).map((tr) => tr.replace(/td|th/g, "").split("<>").filter(Boolean));
  const header = previousTable.shift().map((data) => ({ data, header: true }));
  const rowOfCurrentJob = previousTable.find((tr) => tr[0] === github2.context.job);
  if (rowOfCurrentJob) {
...
async function publishCommentOnPullRequest(token, { accumulatedResult, testResults, conclusion, headSha }) {
  ...
  core4.info(`\u2139\uFE0F - found ${commentsSearch.data.length} comments at ${commentsSearch.url} with status ${commentsSearch.status}`);
  const comments = commentsSearch.data.filter(({ user, body: body2 }) => (user == null ? void 0 : user.login) === "github-actions[bot]" && (body2 == null ? void 0 : body2.startsWith('<h1 id="testim-junit-reporter-msg">')));
  core4.info(`\u2139\uFE0F - found ${comments.length} comments by github-actions, starting with correct HTML`);
  const comment = comments.at(-1);
  const comment_id = comment == null ? void 0 : comment.id;
  const table = buildTableBasedOnPreviousTable(comment == null ? void 0 : comment.body, accumulatedResult);
  const tableMapper = (row) => `<tr>${row.map(rowMapper).join("")}</tr>`;
  const rowMapper = (row) => {
  ...
  if (comment_id) {
    core4.info(`\u2139\uFE0F - Updating existing comment: ${comment_id} on PR: #${prNumber}`);
    await octokit.rest.issues.updateComment({ ...github2.context.repo, comment_id, body });
  } else {
    core4.info(`\u2139\uFE0F - Publishing new comment on PR: #${prNumber}`);
    await octokit.rest.issues.createComment({ ...github2.context.repo, issue_number: prNumber, body });
  }
}
...
async function run() {
  try {
    ...
    core5.startGroup(`\u{1F680} Publish results`);
