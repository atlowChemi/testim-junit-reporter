...
function parseTestimFailureMessage(textContent) {
  ...
  const link = textContent.substring(urlLocation.index + 2).split("https://").at(1);
...
async function parseSuite({ testsuites: { testsuite } = {} } = {}, fileName) {
  ...
  for (const { failure, skipped, name, ["system-out"]: systemOut, classname } of testsuite.testcase) {
    ...
    result.annotations.push({
      ...
      message: parseTestimFailureMessage((failure == null ? void 0 : failure.message) || ""),
...
async function parseTestReports(checkName, summary2, reportPathsGlob) {
  core3.debug(`Process test report for: ${reportPathsGlob} (${checkName})`);
  const res = { annotations: [], checkName, fileName: "", summary: summary2, totalCount: 0, skipped: 0, passed: 0, failed: 0, failedEvaluating: 0 };
